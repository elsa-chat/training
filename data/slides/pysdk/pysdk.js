...
const slides_python_sdk = [
        {
            ...
            content: C.titleSlide(
                ...
                "Interacting with SEMOSS programmatically from Python",
                "60 minutes"
            )
        },
        {
            ...
            content: `
                ...
                <p class="lead">The <code>ai-server-sdk</code> package on PyPI provides a Python client for connecting to a SEMOSS instance, managing sessions, and interacting with all engine types programmatically.</p>
                ${C.code(`pip install ai-server-sdk`, 'bash', 'Install from PyPI')}
                <h3>Core Components</h3>
                ${C.table(
                    ['Class', 'Import', 'Purpose'],
                    [
                        ['<code>ServerClient</code>', '<code>from ai_server import ServerClient</code>', 'Establishes an authenticated connection to a SEMOSS instance. Manages insights, runs Pixel commands, and provides OpenAI-compatible endpoints.'],
                        ...
                        ['<code>DatabaseEngine</code>', '<code>from ai_server import DatabaseEngine</code>', 'Execute SQL queries, insert data, and delete records from SEMOSS database engines.'],
                        ['<code>FunctionEngine</code>', '<code>from ai_server import FunctionEngine</code>', 'Execute pre-defined SEMOSS function engines with custom parameter dictionaries.'],
...
result = server_connection.run_pixel('1+1')`, 'python', 'Connecting to a SEMOSS instance')}
                <h3>OpenAI-Compatible Endpoint</h3>
                <p>The SDK also exposes an OpenAI-compatible endpoint, so you can use the standard <code>openai</code> Python package to interact with any model registered in SEMOSS.</p>
...
)`, 'python', 'Using the OpenAI SDK against SEMOSS')}
